...
function LifeQuest() {
  ...
  const [shieldCharges, setShieldCharges] = useState(() => {
    ...
    const fromItems = JSON.parse(localStorage.getItem("lq2_inv") || "[]").filter((i) => i.effect?.type === "streak_shield" && i.used).length;
  ...
  useEffect(() => {
    localStorage.setItem("lq2_logs", JSON.stringify(logs));
  }, [logs]);
  useEffect(() => {
    localStorage.setItem("lq2_skills", JSON.stringify(unlocked));
  }, [unlocked]);
  useEffect(() => {
    localStorage.setItem("lq2_spused", JSON.stringify(spUsed));
  }, [spUsed]);
  useEffect(() => {
    localStorage.setItem("lq2_health", JSON.stringify(healthChecks));
  }, [healthChecks]);
  useEffect(() => {
    localStorage.setItem("lq2_hitems", JSON.stringify(healthItems));
  }, [healthItems]);
  useEffect(() => {
    localStorage.setItem("lq2_activities", JSON.stringify(activities));
  }, [activities]);
  useEffect(() => {
    localStorage.setItem("lq2_gems", JSON.stringify(gems));
  }, [gems]);
  useEffect(() => {
    localStorage.setItem("lq2_inv", JSON.stringify(inventory));
  }, [inventory]);
  useEffect(() => {
    localStorage.setItem("lq2_lastlogin", lastLogin);
  }, [lastLogin]);
  useEffect(() => {
    localStorage.setItem("lq2_chars", JSON.stringify(ownedChars));
  }, [ownedChars]);
  useEffect(() => {
    localStorage.setItem("lq2_shieldcharges", JSON.stringify(shieldCharges));
  }, [shieldCharges]);
  useEffect(() => {
    localStorage.setItem("lq2_shielddates", JSON.stringify(shieldDates));
  }, [shieldDates]);
  useEffect(() => {
    localStorage.setItem("lq2_equip", equippedChar);
  }, [equippedChar]);
  useEffect(() => {
    localStorage.setItem("lq2_charxp", JSON.stringify(charXP));
  }, [charXP]);
  useEffect(() => {
    localStorage.setItem("lq2_schedule", JSON.stringify(schedule));
  }, [schedule]);
  useEffect(() => {
    localStorage.setItem("lq2_synccode", syncCode);
  }, [syncCode]);
  useEffect(() => {
    localStorage.setItem("lq2_syncat", syncLastAt);
  }, [syncLastAt]);
  useEffect(() => {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { animation: "fadeup .3s ease" } }, /* @__PURE__ */ React.createElement("div", { style: { ...glassCard, padding: "10px 14px", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, letterSpacing: 2, color: "#78716c" } }, "\u672C\u65E5\u306E\u8A18\u9332\u53EF\u80FD\u6642\u9593"), /* @__PURE__ */ React.createElement("button", { onClick: () => setShowScheduleModal(true), style: {
      ...
      const activeBuff = inventory.find((i) => i.effect?.type === "xp_mult" && i.effect.cat === form.cat && i.active && !i.used);
      return activeBuff && /* @__PURE__ */ React.createElement("div", { style: {
        ...glassCard,
        padding: "10px 14px",
        marginBottom: 10,
        ...
        alignItems: "center",
        gap: 8
      } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, activeBuff.icon), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: "#047857" } }, activeBuff.label, "\u30BB\u30C3\u30C8\u4E2D \u2014 \u6B21\u306E\u8A18\u9332\u3067EXP\xD7", activeBuff.effect.val, "\u500D\uFF01"));
